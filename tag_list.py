...
def analyze_tweet_tag_to_plt(tweets,TILL_DAY):
    ...
    flag = 0
    tag_list = []
    dict = read_tags_count(TARGET_TAG_LIST_PATH)
    searched_day = 100000
...
def note_tags(tag_list,TARGET_TAG_LIST_PATH):
    for content in tag_list:
        f = open(TARGET_TAG_LIST_PATH, 'a')  # overwrite
        f.write(content + '\n')
        f.close()
...
TARGET_TAG_LIST_PATH = TARGET_PATH+TARGET+'_tag_list.txt'
...
if __name__ == '__main__':
    ...
    for keyword in keywords:
        ...
        while (True):
            ...
            note_tags(tag_list, TARGET_TAG_LIST_PATH)
            print("tweets:", end="")
            print(tweet_count)
